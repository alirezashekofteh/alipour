...
<div class="col-12">
    <div class="portlet box border shadow">
        ...
        <div class="portlet-body">
            ...
            <form role="form" action="{{ route('admin.post.metapost.update',[$post,$metapost])}}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-body">
                    <div class="form-group curve row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="name" value="{{old('name',$metapost->name)}}" placeholder="نام محتوا">
                        </div>
                    </div><!-- /.form-group -->
                    <div class="form-group curve row">
                        <label class="col-sm-3">متن</label>
                        <div class="col-sm-9">
                            <textarea  class="form-control rtl" id="editor1" name="content" >{{old('content',$metapost->content)}}</textarea>
                        </div>
                    </div><!-- /.form-group -->
                    <div class="form-group curve row">
                        <label class="col-sm-3">کد برنامه نویسی</label>
                        <div class="col-sm-9">
                            <textarea  class="form-control ltr" name="code"> {{old('code',$metapost->code)}} </textarea>
                        </div>
                    </div><!-- /.form-group -->
                    ...
                    <option value="{{ $key }}" {{ $metapost->noe==$key  ? 'selected' : '' }} >{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div><!-- /.form-group -->
                    <div class="form-group curve row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="tartib" value="{{old('tartib',$metapost->tartib)}}"  placeholder="ترتیب">
                        </div>
                    </div><!-- /.form-group -->
                    <div class="form-group curve row">
                    <label class="col-sm-3">بارگزاری فایل</label>
                    <div class="input-group col-sm-9" style="height: 35px">
                        <input type="text" id="image_label" class="form-control ltr" value="{{old('file',$metapost->file)}}"  name="file"
