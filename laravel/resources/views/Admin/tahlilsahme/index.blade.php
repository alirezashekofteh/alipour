              ...
              <div class="col-12">
                <div class="breadcrumb-wrapper">
                  <ol class="breadcrumb p-0 mb-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('admin.saham.index')}}">{{$saham->name}}</a>
                    </li>
...
<hr>
                    <div class="card-body">
                      ...
                      <a href="{{route('admin.saham.tahlilsahme.create',($saham->id))}}" class="btn btn-light-info curve mb-2"><i class="bx bxs-add-to-queue"></i> افزودن تحلیل جدید </a>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($tahlilsahme as $item)
                        <tr class="success">
                            <td>{{ $item->id}}</td>
                            <td>{{ $item->name}}</td>
                            <td>{{$item->saham->name}}</td>
                            <td>{{$item->user->nf}}</td>
                            ...
                            <td>
                                ...
                                <div class="row">
                                    <a href="{{route('admin.saham.tahlilsahme.edit',['saham'=>$saham->id,'tahlilsahme'=>$item->id])}}"><i class="bx bx-edit font-large-1 text-info" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="ویرایش"></i> </a>

                                    <form id="delete-form{{$item->id}}" action="{{ route('admin.saham.tahlilsahme.destroy',[$saham,$item]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                  ...
                  <div class="card-footer">
                    {{ $tahlilsahme->appends([ 'parent' => request('parent') ])->links() }}
                </div>
                </div>
              </div>
          </section>
